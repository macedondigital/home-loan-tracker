...
export default function Dashboard() {
    ...
    try {
      ...
      if (monthsRes.success && monthsRes.data.length > 0) {
        ...
        if (latestSummary?.success) {
          ...
          setAllCategorySummary(latestSummary.data.categories);
        }
      }
    } catch (e) {
  ...
  const nextMoves = buildNextMoves(uploadDays, milestones, allCategorySummary.filter((c) => [...PROBLEM_CATEGORIES, 'retail'].includes(c.id)));
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '0.25rem' }}>
        <h1 style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontSize: '1.5rem', color: '#1a1a1a', marginBottom: '0.25rem' }}>
          {daysLeft} days to go.
        </h1>
      ...
      {nextMoves.length > 0 && (
        <div style={{ ...styles.card, marginBottom: '1.5rem' }}>
          <h2 style={styles.sectionTitle}>Your next moves</h2>
      ...
      <div style={styles.cardGrid}>
        ...
        <div style={styles.card}>
          ...
          {balances && upcoming && upcoming.total_upcoming > 0 && (
            <div style={{ fontSize: '0.75rem', color: '#6b7280', marginTop: '0.25rem' }}>
              After upcoming expenses: {formatCurrency(buffer - upcoming.total_upcoming)}
            </div>
          )}
        </div>
      </div>
